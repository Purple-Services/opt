...
(defn goog-request-route [src-lng src-lat dst-lng dst-lat] 
  (json/read-str (:body (client/get (str "https://maps.googleapis.com/maps/api/directions/json?origin=" src-lat "," src-lng "&destination=" dst-lat "," dst-lng "&sensor=false&key=AIzaSyAFGyFvaKvXQUKzRh9jQaUwQnHnkiHDUCE"))) :key-fn keyword))
...
(defn goog-request-route-with-station [src-lng src-lat dst-lng dst-lat stn-lng stn-lat]
  (json/read-str (:body (client/get (str "https://maps.googleapis.com/maps/api/directions/json?origin=" src-lat "," src-lng "&destination=" dst-lat "," dst-lng "&waypoints=" stn-lat "," stn-lng "&sensor=false&key=AIzaSyAFGyFvaKvXQUKzRh9jQaUwQnHnkiHDUCE"))) :key-fn keyword))
...
(defn suggest-gas-stations-with-driving-time 
  "Return a map in which the keys are station objects and the values are their driving times."
